...
class TokenEvent(BaseModel):
    ...
    # "reasoning" tags a CoT/thinking delta so the frontend routes it to the
    # reasoning panel instead of the message body. Without this field the
    # tag was being silently stripped by create_event_by_type and every
    # streamed thought ended up appended to the assistant text, then
    # overwritten by the final response at end-of-turn — making reasoning
    # look like it only "appeared at the end" instead of streaming live.
...
def create_token(chunk: str, id: str, type: str | None = None) -> TokenEvent:  # noqa: A002
    return TokenEvent(
        chunk=chunk,
        id=id,
        type=type,
